...
function Navbar() {
    ...
    const handleLogin = async (email, password) => {
        try {
            const response = await axios.post('http://localhost:9000/member', { email, password });
            setIsLoggedIn(true);
            setUsername(`Hello, ${extractUsername(response.data.email)}`);
        } catch (error) {
    ...
    return (
        <div className="navbar-container">
            <nav className="navbar">
                <Link to="/">
                    <div className="logo">
                        <img src={icon} className='logoPic' alt="L"></img>
